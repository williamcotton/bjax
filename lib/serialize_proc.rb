...
module ProcSource
  ...
  def handle(proc)
    ...
    while token = lexer.token
      ...
      if [:before_constructor, :check_more].include?(state)
        ...
        if token.is_a?(RubyToken::TkCONSTANT) and
           token.instance_variable_get(:@name) == proc.class.to_s
          # method call?
        ...
        # token is lambda or proc call?
              ...
              [proc.class.to_s.downcase ,"lambda"].include?(token.instance_variable_get(:@name))
          unless state == :check_more
            # okay, code will follow soon.
            state = :before_code
          else
    ...
    if start_token and end_token
      ...
      # lines -- by line_no
      # lines[start_token.line_no..(end_token.line_no - 2)].to_s
      
      # lines -- by seek
      # lines.join("")[(start_token.seek + start_token.name.length)..(end_token.seek - 1)].to_s
      proc.source = lines.join("")[(start_token.seek + start_token.name.length)..(end_token.seek)].to_s
...
class Sproc < Proc
  def source_descriptor
    if md = /^#<#{self.class.to_s}:0x[0-9A-Fa-f]+@(.+):(\d+)>$/.match(old_inspect)
  ...
  def inspect
    if source
      "#{self.class.to_s.downcase} {#{source}}"
    else
      old_inspect
    end
  end
  ...
  def _dump(depth = 0)
    if source
      ...
      raise(TypeError, "Can't serialize #{self.class.to_s} with unknown source code.")
    end
  end

  def to_yaml(*args)
    self.source # force @source to be set
    super.sub("object:#{self.class.to_s}", "#{self.class.to_s.downcase}")
  end

  def self.allocate; from_string ""; end

  def self.from_string(string)
    result = eval("#{self.class.to_s.downcase} {#{string}}")
